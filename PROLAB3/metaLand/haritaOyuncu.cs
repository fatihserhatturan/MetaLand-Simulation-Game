...
namespace metaLand
{
    public partial class haritaOyuncu : Form
    {
        ...
        private void haritaOyuncu_Load(object sender, EventArgs e)
        {
        ...
        private void haritaOyuncu_Load_1(object sender, EventArgs e)
        {
        ...
        public int yöneticiBilgiVer(string talepbilgi)
        {
            ...
            if (talepbilgi == "esya")
            {
                insertQuery = "select * from kullanıcı_info  where kullanıcı_adı like '" + singleton.oyuncuAd + "'";
                talep = "esya_miktarı";
            }
            if (talepbilgi == "yemek")
            {
        ...
        public int yöneticiParaBul()
        {
            ...
            string insertQuery = null;
            insertQuery = "select * from yönetici_info  where yönetici_ad like '" + alanSahip.Text + "'";
            SqlCommand cmd = new SqlCommand(insertQuery, connection);
            SqlDataReader dr = cmd.ExecuteReader();
        ...
        private void ürünAl_Click(object sender, EventArgs e)
        {
            ...
            if (tür.Text == "magaza")
            {
                ...
                insertQuery = "Update kullanıcı_info set esya_miktarı  = @esya where kullanıcı_ad = @ad ";
                connection.Open();
                SqlCommand cmd = new SqlCommand(insertQuery, connection);
            ...
            if (tür.Text == "market")
            {
                ...
                insertQuery = "Update kullanıcı_info set yemek_miktarı  = @yemek where kullanıcı_adı = @ad ";
                connection.Open();
                SqlCommand cmd = new SqlCommand(insertQuery, connection);
        ...
        private void isBasvuruOnayla_Click(object sender, EventArgs e)
        {
            SqlConnection connection = connectionstring;
            connection.Open();
            ...
            cmd.Parameters.AddWithValue("isim", singleton.oyuncuAd);
        ...
        public void meslekEkle()
        {
            SqlConnection connection = connectionstring;
            connection.Open();
            string insertQuery = "Update kullanıcı_info set kullanıcı_meslek  = @meslek where kullanıcı_adı = @ad ";

            SqlCommand cmd = new SqlCommand(insertQuery, connection);
            cmd.Parameters.AddWithValue("meslek",tür.Text);
            cmd.Parameters.AddWithValue("ad", singleton.oyuncuAd);
            cmd.ExecuteNonQuery();
            connection.Close();
        }
